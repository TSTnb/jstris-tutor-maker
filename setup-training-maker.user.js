...
async function newTrigger(triggerType, triggerArg) {
    document.querySelector('a[data-field-type=trig]').click();
    await sleep();
...
async function newQueueChange(queue, replace = false, repeat = false) {
    ...
    document.querySelector('a[data-field-type="queue"]').click()
    await sleep();
...
window.newMapIndex = 0;
window.newMapContent = undefined;
...
async function syncMaps() {
    ...
    window.newMapContent = changedInput.value;
    ...
    for (const map of maps) {
        if (changedMap === map) {
            foundMapYet = true;
            window.newMapIndex = Math.floor(index / 2);
            if (isReplacementMap) {
                await updateMapContent(previousMap, window.newMapContent)
            }
        ...
        index++;
        await updateMapContent(map, window.newMapContent)
    }
    clearInterval(progressInterval);

    window.newMapIndex++
    const subtractionMaps = getSubtractionMaps();
    if (window.newMapIndex < subtractionMaps.length) {
        subtractionMaps[window.newMapIndex].querySelector(':scope a.open-map-edit').click();
    }
    await resetStatus();
}
...
async function newMap(mapType) {
    document.querySelector('a[data-field-type=map]').click();
    await sleep();
...
async function newRuleset(rulesetType) {
    document.querySelector('a[data-field-type=rule]').click();
    await sleep();
...
async function newCondition(conditionType, conditionValue, doIfTrue, conditionDo) {
    document.querySelector('a[data-field-type=cond]').click();
    await sleep();
...
async function cycle(blockCount, queue) {
    ...
    await newTrigger(TriggerTypeOnSpecificBlockNumber, blockCount * 2 - 1);
...
async function makeCycles(totalBlocks, queue) {
    ...
    getSubtractionMaps()[window.newMapIndex].querySelector(':scope a.open-map-edit').click();
    setMapSubmitButtonText();
}

async function initUserMode() {
    await newTrigger(TriggerTypeBeforeGame);
}
